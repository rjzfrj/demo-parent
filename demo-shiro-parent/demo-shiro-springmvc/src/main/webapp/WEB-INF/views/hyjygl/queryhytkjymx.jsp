...
$('#userTable').datagrid({
			...
			columns:[[
			    ...
			    {field:'id',title:'商户退款单号',sortable:true,
					formatter:function(value,row,index){return row.id;} 
				},
				{field:'source_serial',title:'商户订单号',sortable:true,
					formatter:function(value,row,index){
						return row.source_serial;  
					}
				},
				{field:'serial',title:'退款流水号',sortable:true,
					formatter:function(value,row,index){
						return row.serial;  
					}
				},
				{field:'order_amt',title:'原订单金额(元)',sortable:true,
					formatter:function(value,row,index){
				...
				{field:'tran_amt',title:'实际退款金额(元)',sortable:true,
					formatter:function(value,row,index){
						return row.tran_amt; 
					}
				},
				{field:'business_result',title:'交易状态',sortable:true,
					formatter:function(value,row,index){
					    ...
					    if(row.business_result=="00")
					      s="处理中";
					    else if(row.business_result=="03")
					      s="退款成功";
					    else if(row.business_result=="04")
					      s="退款失败";
						return s;  
					}
				},
				{field:'operator',title:'详细',sortable:true,
					formatter:function(value,row,index){
						var i = '<a href="javascript:void(0)" onclick="queryDetail(\''+row.id+'\')">查看</a> ';
						return i;
					}
				}
			]],
   ...
   function exportUser(){
        ...
        window.location.href="${request.getContextPath()}/business/memberTk_export"+params;
	...
	function queryDetail(id){
		$.ajax({type:"post",url:"getTkSIGN_IMAGE?id="+id,  
...
<form id="queryForm">
   <div class="title_right"><strong>退款交易明细查询列表</strong></div>  
     ...
     <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">会员卡号：</td>
	     ...
	     <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">商户订单号：</td>
	     <td><input type="text" id="source_serial" name="source_serial" class=" span1-1" /></td>
     </tr>
     <tr>
	     <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">商户退款单号：</td>
	     <td><input type="text" id="id" name="id" class=" span1-1" /></td>
         <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">退款流水号：</td>
   ...
   <div id="dd" class="easyui-dialog" title="" closed="true">
     ...
     <table class="table table-bordered"> 
     <tr>
       <td><div style="width:100px"><img src="" id="SIGN_IMAGE"/></div></td>
     </tr>
   </table>
   </div>
