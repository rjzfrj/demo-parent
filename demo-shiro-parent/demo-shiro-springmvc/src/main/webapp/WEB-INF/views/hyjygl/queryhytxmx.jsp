...
$('#userTable').datagrid({
			...
			columns:[[
				...
				{field:'member_card',title:'会员账号',sortable:true,
				...
				{field:'id',title:'商户订单号',sortable:true,
				...
				{field:'chnOrderNo',title:'渠道交易流水号',sortable:true,
					formatter:function(value,row,index){
						return row.chnOrderNo; 
					}
				},
				{field:'order_amt',title:'订单金额(元)',sortable:true,
					formatter:function(value,row,index){
						return row.tranAmt;  
					}
				},
				...
				{field:'tranSource_DS',title:'支付方式',sortable:true,
					formatter:function(value,row,index){
						 var s ="";
				...
				{field:'business_result',title:'交易状态',sortable:true,
					formatter:function(value,row,index){
					    ...
					    if(row.tranStatus=="00")
					      ...
					      s="交易失败";
					    else if(row.tranStatus=="03")
					      s="退款成功";
					    else if(row.tranStatus=="04")
					      s="退款失败";
						return s; 
					}
				}
   ...
   function exportUser(){
        ...
        window.location.href="${request.getContextPath()}/business/memberTx_export"+params;
	...
	function queryDetail(id){
		$.ajax({type:"post",url:"getXfSIGN_IMAGE?id="+id,  
...
<form id="queryForm">
<div class="title_right"><strong>会员提现明细查询</strong></div>  
	...
	<table class="table table-bordered">
		<tr>
			...
			<td><input type="text" id="merchant_code" name="merchant_code" class=" span1-3" maxlength="16"/></td>
			<td align="right" nowrap="nowrap" bgcolor="#f1f1f1">会员卡号：</td>
			<td><input type="text" id="member_card" name="member_card" class=" span1-3" maxlength="12"/></td>
		</tr>
		<tr>
			<td align="right" nowrap="nowrap" bgcolor="#f1f1f1">商户订单号：</td>
			<td><input type="text" id="id" name="id" class=" span1-3" /></td>
			...
			<td><select id="business_type" name="business_type" style="height:25px">
					<option value="">全部</option>
					<option value="00">钱包支付</option>
					<option value="01">刷卡支付</option>
				</select>
			</td>
