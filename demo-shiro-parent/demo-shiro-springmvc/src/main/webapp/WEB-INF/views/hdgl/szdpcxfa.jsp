 ...
 function delSchema(id){
  ...
  function dpSchemaTj(){
    ...
    // if(mydate>start){
      ...
      // return;
    // }
	 var url= "${request.getContextPath()}/activity/setDpSchema";
     ...
     success: function(msg){
       if(msg=="success"){
         window.location.href="${request.getContextPath()}/activity/dpSchemaFowd";
       }else if(msg=="checkSuccess"){
         alert("该点评优惠已存在,请重新输入");
       }
      }
     });
	}
...
<form id="queryForm">
   <div class="title_right"><strong>设置点评促销方案</strong></div>  
     ...
     <input type="text" class="laydate-icon span2-1" id="endDate" name="endDate" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})"/>
        ...
        <table  class="margin-bottom-20 table  no-border" >
     	...
     	<input type="button" value="新增" class="btn btn-info " style="width:80px;" onclick="openDp();"/>
     	</td>
     </tr>
 </table>
  ...
  <div id="dd" class="easyui-dialog" title="点评方案" closed="true">
     ...
     <tr>
       <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">点评赠送优惠金额：</td>
       <td>
   ...
   <table class="margin-bottom-20 table  no-border" >
        <tr>
     	<td class="text-center"><input type="button" value="确定" class="btn btn-info " style="width:80px;" onclick="dpSchemaTj();"/>
