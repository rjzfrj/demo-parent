...
$('#userTable').datagrid({
 ...
 function delSchema(id){
   ...
   function zcSchemaTj(){
	  var reward = $("input[name='reward']");
	  var reg =/^\d+$/;
	  for(var i=0;i<reward.length;i++){
	     var amount = $.trim(reward[i].value);
    ...
    // if(mydate>start){
     ...
     //  return;
   //  }
	 var url= "${request.getContextPath()}/activity/setZcSchema";
     ...
     success: function(msg){
       if(msg=="success"){
         window.location.href="${request.getContextPath()}/activity/zcSchemaFowd";
       }else if(msg=="checkSuccess"){
         alert("该注册优惠已存在,请重新输入");
       }
      }
     });
	}
...
<body>
<form id="queryForm">
   <div class="title_right"><strong>设置注册促销方案</strong></div>  
     ...
     <input type="text" class="laydate-icon span2-1" id="endDate" name="endDate" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})"/>
        ...
        <table  class="margin-bottom-20 table  no-border" >
     	...
     	<input type="button" value="新增" class="btn btn-info " style="width:80px;" onclick="openZc();"/>
     	</td>
     </tr>
 </table>
  ...
  <div id="dd" class="easyui-dialog" title="注册方案" closed="true">
     <form action="${request.getContextPath()}/activity/setZcSchema" method="post" id="userForm">
     ...
     <tr>
       <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">注册赠送优惠金额：</td>
       <td>
       <input type="text" name="reward" id="reward" class="span2" />
       </td> 
     </tr>
      <tr>
   ...
   <table class="margin-bottom-20 table no-border" >
        <tr>
     	<td class="text-center"><input type="button" value="确定" class="btn btn-info " style="width:80px;" onclick="zcSchemaTj();"/>
