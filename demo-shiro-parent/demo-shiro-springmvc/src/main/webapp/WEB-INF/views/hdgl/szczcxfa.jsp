...
$('#userTable').datagrid({
			...
			columns:[[
				...
				{field:'rule',title:'充值金额(元)',sortable:true,
				...
				{field:'start_time',title:'生效时间',sortable:true,
					formatter:function(value,row,index){
 ...
 function delSchema(id){
	...
	//返回
   function comBack(){
       //window.location.href="${request.getContextPath()}/activity/czSchemaFowd";
       $('#dd').dialog('close');
	}
	...
	function addXf(){
	   i++;
	   var s = "sub"+i;
	   var domEle = "<div id='xf'>充值金额:"+"<input type='text' name='czAmount' id='czAmount' class='span1'/>"+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;优惠券金额:"+"<input type='text' name='czAmount' id='czAmount' class='span1-1'/>"+"&nbsp;<input type='button' name="+s+" id="+s+" class='span1' value='-' onclick='delXf(this.id)'/></div>";
       $("#xfAdd").after(domEle);
	}
	function delXf(s){
	 $('#'+s).parent().remove();
	...
	function czSchemaTj(){
	  var czAmount = $("input[name='czAmount']");
	  var reg =/^\d+$/;
	  for(var i=0;i<czAmount.length;i++){
	     var amount = $.trim(czAmount[i].value);
    ...
    /// }
     ...
     //if(mydate>start){
      ...
      // return;
    // }
	 var url= "${request.getContextPath()}/activity/setCzSchema";
...
<form id="queryForm">
   <div class="title_right"><strong>设置充值促销方案</strong></div>  
     ...
     <input type="text" class="laydate-icon span2-1" id="endDate" name="endDate" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})"/>
        ...
        <table  class="margin-bottom-20 table  no-border" >
     	...
     	<input type="button" value="新增" class="btn btn-info " style="width:80px;" onclick="openCz();"/>
     	</td>
     </tr>
 </table>
...
<div id="dd" class="easyui-dialog" title="充值方案" closed="true">
     ...
     <div id="xfAdd">       
                  充值金额:<input type="text" name="czAmount" id="czAmount" class='span1'/>
       &nbsp;&nbsp;&nbsp;&nbsp;优惠券金额:
       <input type="text" name="czAmount" id="czAmount" class='span1-1' />
       <input type="button" name="add" id="add" class="span1" value="+" onclick="addXf();"/>
     </div>
     -->
     <table class="table table-bordered">
	 <tr>
       <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">充值金额：</td>
       <td><input type="text" name="czAmount" id="czAmount" class='span2'/></td>
       <td align="right" nowrap="nowrap" bgcolor="#f1f1f1">优惠券金额：</td>
       <td>
       <input type="text" name="czAmount" id="czAmount" class='span1-1' />
       </td> 
     </tr>
   ...
   <table class="margin-bottom-20 table  no-border" >
        <tr>
     	<td class="text-center"><input type="button" value="确定" class="btn btn-info " style="width:80px;" onclick="czSchemaTj();"/>
